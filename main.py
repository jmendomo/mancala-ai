...
class state():
    ...
    def result(self, action):
        ...
        #================================================
        if self.result_of[action]:
            return self.result_of[action]
        ...
        start_pit = action + 1
        end_pit = action + board[action]
        
        # make sure we don't accidentally fill-in the opponent's store
        pit_to_skip = -1
        if action < 6 and end_pit >= 13:
            pit_to_skip = 13
        if action > 6 and (end_pit % NUM_PITS) >= 6
            pit_to_skip = 6
        ...
        self.result_of[action] = next_state
    ...
    def is_valid_action(self, action):
        return ((action >= 0 and action < 13) and
                (action != 6) and
                (self.board[action] != 0))
